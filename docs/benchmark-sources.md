...
- [2024 JAINWON 공개 기업자료](https://recruit.jobda.im/hubfs/TREND%20REPORT_HR%20%EA%B3%A0%EB%AF%BC%EC%9E%88%EC%8A%B5%EB%8B%88%EB%8B%A4_2%ED%8E%B8.pdf)는 PDF 10쪽에서 9개 게임을 모두 4분으로 제시하고, PDF 12쪽의 약속 정하기 화면에는 `17/남은 문항 6`과 버스 답안 6개가 보인다. 2023 레거시 영상의 약 4분·라운드별 `1/남은 문항 9`·버스 5선지와 충돌하므로 서로 다른 공개 버전으로 기록하고, 실제 응시에서는 기업 초대 안내를 우선한다.
...
| PM | 약속 정하기 | 총 4라운드, 라운드당 10문항. 1R 요일 → 2R 위치 → 3R 메뉴 → 4R 버스 순서다. 1–3R은 모두가 고른 항목을 찾으며 초반에는 친구마다 3개, 후반에는 4개가 제시된다. 4R은 누구도 타지 않은 버스를 고른다. | 약 4분 / 마우스 | `ROUND_INTRO → SHOW_1 → SHOW_2 → SHOW_3 → QUERY → CHOOSE` |
...
게임별 표의 기본 총시간은 2023 공개 레거시 카드의 “약” 표기다. 다만 길 만들기는 같은 시기 공식 영상에서 5분으로 안내해 카드의 약 3분과 상충한다. 2024 공개 기업자료도 9개 모두 4분으로 달라 현행 설정으로 단정할 수 없다. 약속 정하기의 2023 영상은 4라운드와 라운드당 10문항을 공개하지만, 정확한 3개→4개 전환 문항은 공개하지 않는다. 그 밖의 세부 문항 수, 오답 피드백, 점수식과 대부분의 자극 노출시간도 공개되지 않았다. 개수 비교하기는 2023 공식 영상에서 단어 1초 제시·생각 3초를 별도로 공개한다.
...
[2023 JOBDA 공식 해설 영상](https://www.youtube.com/watch?v=FTo_F_Yz4g4)은 `00:14`에 총 4라운드를 명시하고 요일 → 위치 → 메뉴 → 버스 순서를 설명한다. `02:57`의 3라운드 메뉴 예시와 `03:46`의 4라운드 버스 예시는 각각 문항 `1`, `남은 문항 9`로 시작하므로, 이 공개 레거시 구조는 **각 라운드 10문항, 총 40문항**이다.

| 라운드 | 사람별 자극 | 질문/버튼 | 정답 조건 |
|---|---|---|---|
| 1 요일 | 친구 3명의 선호 요일. 초반에는 각 3개, 후반에는 각 4개. | 월–일 7개 버튼 | `intersection(S1,S2,S3)`의 공통 요일 |
| 2 장소 | 4×4=16칸 장소판에서 친구 3명의 선호 위치. 초반에는 각 3곳, 후반에는 각 4곳. | 16칸 장소 선택 | `intersection(S1,S2,S3)`의 공통 칸 |
| 3 메뉴 | 친구 3명의 메뉴 이미지. 초반에는 각 3개, 후반에는 각 4개. | 2023 영상 예시의 메뉴 이미지 6개 | `intersection(S1,S2,S3)`의 공통 메뉴 |
| 4 버스 | 친구 3명이 타고 온 버스 번호 묶음. | 2023 영상 예시의 후보 번호 5개 | `choices \ union(S1,S2,S3)`의 누구도 타지 않은 번호 |

`후반`이 몇 번째 문항부터인지는 공식 영상과 번들에 숫자로 공개되지 않았다. 10문항을 기계적으로 5개/5개로 나누는 것은 구현 추정값이며 공식 규칙으로 표기하지 않는다.

### 4.2 공식 자료별 버전 경계

| 공식 자료 | 직접 확인한 화면·문구 | 구현에 사용할 때의 경계 |
|---|---|---|
| [2023 JOBDA 공식 해설 영상](https://www.youtube.com/watch?v=FTo_F_Yz4g4) | `00:14` 총 4라운드와 유형 순서, `02:28` 전반 3개·후반 4개 설명, `02:57` 메뉴 라운드의 `1/남은 문항 9`, `03:46` 버스 라운드의 `1/남은 문항 9`와 5선지 | 2023 공개 레거시 모드는 4라운드×10문항으로 구현할 수 있다. 정확한 3→4 전환 문항, 사람별 노출시간, 실시간 정오 피드백은 미공개다. |
| [공식 튜토리얼 미리보기 `PM.mp4`](https://jobda.acca.ai/static/video/tutorial/PM.mp4) | 21.77초 편집 클립에서 `00:00` 요일 `1/남은 9`, `00:08.506` 위치 `2/남은 8`, `00:13.013` 메뉴 `3/남은 7`, `00:16.836` 버스 `4/남은 6`; `00:20.393` 버스 5선지 | 네 유형을 짧게 이어 붙인 **미리보기**다. 이 숫자 흐름을 전체 게임의 혼합 순서나 총 10문항 근거로 사용하지 않는다. 라운드 구조는 위 공식 해설 영상을 우선한다. |
| [2024 JAINWON 공개 기업자료](https://recruit.jobda.im/hubfs/TREND%20REPORT_HR%20%EA%B3%A0%EB%AF%BC%EC%9E%88%EC%8A%B5%EB%8B%88%EB%8B%A4_2%ED%8E%B8.pdf) | PDF 10쪽은 약속 정하기를 포함한 9개 게임을 모두 4분으로 표기하고, PDF 12쪽 화면은 `17/남은 문항 6`과 버스 답안 6개를 보여준다. | 2023의 라운드별 `1/남은 문항 9`·버스 5선지와 다른 공식 공개 버전이다. 한 장의 화면만으로 2024 전체 라운드 구조를 추정하지 않으며, 5선지와 6선지를 한 모드에 섞지 않고 버전 설정으로 분리한다. |
...
공식 영상 화면에서는 별도 제출 버튼보다 선택 버튼이 답안 입력 역할을 한다. 2023 공개 레거시의 시도 수는 라운드당 10문항으로 확인되지만, 정확한 노출시간, 3→4 전환 문항, 오답 피드백 시간, 사람별 버스 수는 공개되지 않았으므로 설정값으로 둔다.
...
| 현재 상태 | 이벤트 | 가드 | 효과 | 다음 상태 |
|---|---|---|---|---|
| `PUZZLE_READY` | 셀/대각 클릭 | 클릭 예산 > 0(정책이 확인된 버전에만 적용) | 울타리 설치·교체·제거, 예산 갱신 | `EDIT_BOARD` |
| `EDIT_BOARD` | 셀/대각 클릭 | 클릭 예산 > 0(정책이 확인된 버전에만 적용) | 보드 갱신 | `EDIT_BOARD` |
...
위 표의 클릭 예산 가드는 2023 공개 영상 화면에서 관찰된 상태를 모델링한 것이다. 초기값과 설치·교체·삭제·초기화의 차감·복원 정책은 공개 자료로 확정되지 않았으므로, 현재 앱은 하드 제한을 임의 구현하지 않고 실제 조작 수만 기록한다.
...
| 16 | JOBDA 공식 영상 | A1 | <https://www.youtube.com/watch?v=FTo_F_Yz4g4> | `00:14`의 4라운드, `02:57`·`03:46`의 라운드별 10문항 시작 화면, 전반 3개·후반 4개와 버스 5선지를 확인한다. |
...
| 44 | JOBDA Recruit 공식 PDF | A1 | <https://recruit.jobda.im/hubfs/TREND%20REPORT_HR%20%EA%B3%A0%EB%AF%BC%EC%9E%88%EC%8A%B5%EB%8B%88%EB%8B%A4_2%ED%8E%B8.pdf> | PDF 10쪽의 게임별 4분 표기와 PDF 12쪽 약속 정하기 버스 6선지 화면을 2024 공식 공개 버전 근거로 분리한다. |
