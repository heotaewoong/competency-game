...
## 21:00 후속 — 도형 묶음 카드 최소 폭과 진입 지연 내부 계측

- 21:00:52 KST 시작, 원본 `c939375` clean·origin 확인. 앞선 문서 CI35995039146 성공 확인, 공개 alias는 제품 `9a63f44`/`dpl_GZhGYi5LAqq4aBrncKB6WF4mLFb5` READY 유지. 기존 4187 서버나 진행 중 배포 없음. 종료 시각은 사용자 지시대로2026-09-25 00:00 KST다.
- C:\TEMP 전용 임시 계측으로 dynamic import 시작/완료·GameStage 첫 render/layout/effect·rAF·MutationObserver의 DOM 삽입 시각을 측정했다. 첫 실행 Chromium/WebKit 각각1개 **2/2 통과(20.8초)**. Chromium render→layout245.5ms, WebKit4772ms로 차이가 있었고 WebKit 게임 청크 수신은7/11ms였다. WebKit click→stage DOM은5101ms지만 Playwright visible 검사는 click 명령 반환 뒤 시작하므로5초 검사 통과와 실제 사용자5초 미만을 동일시하지 않는다. 순수 CPU/페인트 시간 또는 원인 확정은 아니다.
- render 본문 끝·마지막 자식 render/layout을 추가한 두 번째 계측은 **1통과/1실패(16.2초)**. Chromium 본문0.3ms·본문 끝→마지막 자식19.9ms·마지막 자식→layout222.7ms. WebKit은 기록 준비5초에서 실패하여 게임 클릭·새 stage 계측 미실행. 별도 동일 WebKit 재검사도 **0통과/1실패**이고 같은 초기 경계다. 마지막 초기 JS 수신은191/206ms였으나 기록 준비 DOM은5881/6344ms에 나타났다. 실패 이후 snapshot의 busy=false를 검사 성공으로 바꾸지 않는다. 더 반복하거나 timeout을 늘리지 않았다.
- 증거: `C:\TEMP\cg-stage-load-20260924-console.log`, `cg-stage-load-fine-20260924-console.log`, `cg-stage-load-fine-followup-20260924-console.log`와 각 동명 결과 폴더. 임시 진단/계측 앱 원본은 `C:\TEMP\cg-stage-load-sources-20260924`에 보존했다. 계측은 G:나 배포에 넣지 않았으며 종료 후 검증본 앱2파일에서 제거하고 원본 SHA256 일치를 확인했다. 사용자 앱 종료·전역/인증 설정 변경 없음. 21:15경 여유 메모리1070468KB/총16119016KB를 관찰했지만 원인으로 단정하지 않는다.
- 확인된 제품 수정은 `app/globals.css`의 NBack 묶음 grid 한 줄뿐이다.3열 고정 대신 카드 최소180px 기반 자동 열을 사용하고 기존<=620px 모바일2열·세로 카드 규칙을 유지한다. 타이머·게임 규칙·데이터·의존성 변경 없음(Ponytail lite). 독립 읽기 검토에서 신규 P1/P2 위험 없음.
- 신규 `e2e/nback-group-layout.spec.ts`6개:1265×712, 실제 컨테이너979/981px, viewport620/621px,383px. 제목/본문 실제 글자 영역·줄 수·미리보기15개·겹침/잘림/넘침·모바일2열·키보드 선택/저장을 검사한다. 기존 Desktop WebKit 프로젝트에도 등록했다. 수정 전 공개1265 검사는 **예상 실패(RED)**: 제목 contentWidth0px, 자동 선택4줄/60px 등 원래 문제를 재현했다. 증거 `C:\TEMP\cg-nback-layout-red-20260924` 및 형제 `-report`. 수정본 검증과 공개 배포는 아직 완료로 보고하지 않는다.
- 첫 최소 폭 수정의 로컬 IAB 화면에서 ‘자동 선택’이 단어 중간에서 꺾여, 같은 묶음 카드 제목의 기존 CSS 선택자에 `word-break: keep-all`만 추가했다. 최종 제품 변경은 CSS2줄이다. 타입·전체 lint·단위203/203·production build 통과했으며 CSS 최종 보완 후 build도 재통과했다. 검증 복사본의 오래된 Git 오브젝트 누락으로 해당 복사본의 `git diff --check`는 실패했으나, 권위 원본 G:의 diff check는 통과하고 소스5개 SHA256도 비교했다. C:의 Git metadata는 변경/복구하지 않았으며 커밋은 G:에서만 한다.
- 세부 계측 독립 해석은 `C:\TEMP\cg-stage-load-sources-20260924\MEASUREMENTS.md`에 보존했다. `stage-layout`은 useLayoutEffect 시점이지 CSS layout CPU profiler가 아니고, WebKit LongTask API는 미지원이다. 최신 실패2개에서 게임 단계 계측은 미도달이므로 최초4772ms 구간의 세부 원인은 열린 항목으로 남긴다.
- 최종 로컬 Chromium·Desktop WebKit **17/17 통과(1.7분)**: 새 묶음 레이아웃 각6개, 무제한 설정/수동 완료·저장 각2개, Chromium NBack pause/겹친 만료1개. `C:\TEMP\cg-nback-layout-local-20260924`와 형제 `-report`. 기존 timeout/retry 설정 유지, 런타임 오류0. 실제 IAB 데스크톱에서도6개 카드의 제목·암기명·도형·자연스러운 ‘자동/선택’ 줄바꿈을 캡처 확인하고 console error0. 다른 사용자 설정·기록을 변경하지 않았다.
- 제품 CSS·검사·config·문서4개만 커밋/push하고 전체 원격 CI를 검증한다. 후보를 만들더라도 CI 성공 전에는 지정 공개 alias를 승격하지 않는다. 임시 진단은 커밋/배포 대상이 아니며 아직 공개 반영 완료로 보고하지 않는다.
- 제품 `fb8cbb469d64d12bb3dfb6fb3f88be8099f3c030` main push 완료. [CI35998858039](https://github.com/heotaewoong/competency-game/actions/runs/35998858039) 진행 중이며 전체 등록 검사는157개다. clean·동일 SHA 릴리스 복사본에서 후보1개 생성: `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP`, `https://competency-game-dz9s9z25k-heotaewoongs-projects.vercel.app`. READY/production·commit metadata 일치·인증 HTTP200, Next16.3.3/Node22·빌드33초·npm audit0. eslint9 지원 종료 경고는 별도 의존성 유지보수 사항이며 빌드 실패로 간주하지 않는다. 아직 지정 공개 alias는 이전 검증본이다.
- 로컬 IAB383×778에서도6개 카드의 기존2열·제목/도형/암기명 세로 순서·고정 하단 시작 버튼을 직접 캡처 확인했다. console error0·viewport reset·임시 탭/소유4187 서버 종료 완료. [JOBDA 도형 순서 공식 해설](https://www.jobda.im/info/341)의2023-04-01 게시물과 공식 영상 연결도 재확인했다. 이번에는 영상을 다시 전부 검토하지 않았고 기존 공개 규칙 경계를 유지하며 비공개 현행 사양 확인으로 표현하지 않는다.
- 다음 지연 계측 후보(원인 미확정): 최초 root layout 전 실행 가능한 사용자 동기 I/O는 `RuleCheck.initialCompletion`의 storage get1개다. 빈 저장소는 JSON 파싱을 생략한다. intro의33개 canvas 그리기는 passive effect여서 첫 layout 뒤이고, 하위 사용자 layout effect/ref callback은 없다. 후속은 최초 렌더의 RuleCheck storage 시작/종료, 기존 body/tail 표시, tail 뒤1회 event-loop 도달 표시를 결합해 실제 동기 작업과 중간 대기를 분리한다. 계측 부하와 Windows 호스트 영향을 포함해 해석하며 추측성 앱 변경은 하지 않는다.
- [CI35998858039](https://github.com/heotaewoong/competency-game/actions/runs/35998858039) **성공(14분51초)**: 타입·전체 lint·단위·production build·브라우저 **157/157(13.1분)**, flaky/재시도 없음. 지정 alias가 이전 배포를 유지함을 확인한 뒤 동일 후보 `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP`를 한 번 promote했다. 공개 alias readback 일치·HTTP200/redirect0 확인. 공개 별도 Chromium/Desktop WebKit/Mobile Safari30개 회귀검사는 진행 중이다. 이 결과가 초기 WebKit 지연 해결을 뜻하지 않는다.
- 공개 최종 **30/30 통과(2.9분)**: Chromium13·Desktop WebKit12·Mobile Safari5. 새 묶음 카드6조건/양쪽 엔진, 기존383×778·844×360·1280×900 Tab 초점, 무제한 설정·수동 진행/저장, NBack pause/겹친 만료, 모바일 준비센터·터치 진입·기록 이전을 검증했다. 증거 `C:\TEMP\cg-nback-layout-production-20260924`와 형제 `-report`. 이번 공개 실행 성공이 과거 지연 실패나 이번 임시 진단 실패2개를 없애지 않는다. 실제 iPhone 기기는 미검증이다.
- 공개 IAB의 진행 중 게임/모달이 없음을 먼저 확인하고 새로고침했다.383×778에서 NBack 세부 설정→ArrowRight로 자동 선택에서 묶음1 선택·초점 표시→1265×712로 변경해ArrowLeft로 원래 자동 선택 복원·수정된2열/한글 줄바꿈/도형을 직접 확인했다. Esc 후 원래 카드 초점·inert0·기존 기록5회 유지·console error0을 확인하고 viewport override를 reset했다. 게임 완료/기록 추가·삭제 없음. 최근30분 Vercel error 로그 없음(Drains 미조회).
- 최종 제품은 `fb8cbb4`/`dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP`이며 지정 공개 주소에 반영됐다. 소유한 로컬 서버·테스트·배포 프로세스는 모두 종료했고 임시 탭 정리 완료. 문서 후속 커밋만 push하고 재배포하지 않는다. 다음 heartbeat는 문서 CI/Git/alias 재확인 후 위 WebKit 지연 원인 분리를 이어간다. 게임 규칙·타이머·보존 데이터·의존성을 임의 변경하지 않았으며, 무오류·비공개 실전과 완전 동일함을 보장하지 않는다.2026-09-25 00:00 KST 종료와 자동화22 ACTIVE 유지.

## 22:01 후속 — 저장소·렌더·검사 계측 부하 분리

- 22:01:23 KST 시작. 원본 `b24d06b` clean·origin 확인, 앞선 문서 CI36001252064 성공. 지정 공개 alias는 `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP` 유지·HTTP200/redirect0·최근30분 Vercel error로그 없음(Drains 미조회). 진행 중 서버·앱 변경·배포 없음. 종료는 연장된2026-09-25 00:00 KST를 유지한다.
- C:\TEMP 검증본의 Home/GameStage/RuleCheck에만 임시 performance mark를 넣었다. 별도 진단은 홈5초 검사와 홈 준비 후 게임5초 검사를 분리했다. 후자의 홈 준비15초는 설정 전제이며 홈5초 성능검사 통과로 간주하지 않는다. Storage.getItem은 키·값 없이 호출 시각/소요시간만 최대100개 측정하고 원메서드·this·return/throw를 보존했다. rAF 루프·앱 타이머·새 의존성·원본 G: 계측 변경 없음. Ponytail lite에 따라 기존 진단을 재사용했다.
- 최초 Chromium/WebKit4개 실행은 **3통과/1실패(31.9초)**. WebKit 홈5초는 실패했으며 사후 snapshot의 ready를 성공으로 바꾸지 않는다. 이 샘플의 Home render→layout4492ms, 복원 effect 본문5ms, rAF 대기108ms/본문5ms, getItem13회 합2ms였다. 실패 시점의 긴 구간은 저장소 복원 전에 있었다.
- 별도 WebKit 게임 샘플은 actual click→DOM4496ms, render→layout3631ms. body 끝→RuleCheck1636ms, RuleCheck 전체 저장 판정0ms, RuleCheck 끝→tail1957ms였다(0은 계측 정밀도 내).20개 getItem도 모두0ms. tail에서 예약한1회 task는 layout 전에 실행됐다. 따라서 사용자 저장소 함수를 긴 동기 지연 원인으로 볼 근거는 없으며, render→layout 전체를 연속 CPU 차단으로 표현하지 않는다. WebKit LongTask API는 미지원이라 앞쪽의 CPU/React 양보/호스트 대기는 분리하지 못했다.
- [Playwright 공식 성능 주의](https://playwright.dev/docs/best-practices#debugging-on-ci)를 대조하고, 동일 앱·검사·영상 설정에서 trace만 끈 비교를 수행했다. WebKit2개 **2/2 통과(10.1초)**, strict Home render→layout118ms, 게임 click→DOM327ms였다. 그러나 trace를 원래대로 켠 재검사도 **2/2 통과(12.2초)**, 게임 click→DOM322ms였으므로 **trace를 단독 원인으로 확정하지 않는다**. 재검사에는 홈 rAF 대기963ms와 클릭 명령 내부 지연도 관찰되어 순서·캐시·호스트 변동을 배제하지 못했다. 앱이나 timeout을 바꾸지 않았고 더 반복해 성공만 고르지 않았다.
- 증거는 `C:\TEMP\cg-storage-stage-20260924-console.log`, `cg-storage-stage-no-trace-20260924-console.log`, `cg-storage-stage-trace-recheck-20260924-console.log`와 각 동명 결과 폴더에 보존했다. 임시 진단2개와 계측 앱3개는 `C:\TEMP\cg-storage-stage-sources-20260924`에 별도 보존했다.3회 실행 모두 runtime error 배열은 비어 있으나 첫 준비검사 실패는 유지한다. Windows WebKit 자동화 관측이며 실제 iPhone 성능/무오류 보장이 아니다.
- 임시 앱 계측을 모두 제거하고3개 파일이 G: 원본과 텍스트가 같음을 확인한 뒤 원본 줄바꿈까지 복사해 SHA256 일치를 복구했다. 검증 경로의 진단2개도 보존본 확인 후 제거했다. G: 제품 코드·게임 규칙·타이머·보존 데이터·의존성은 이번에 바꾸지 않았고 사용자 앱 종료나 전역/인증 설정 변경도 하지 않았다. 정상 원본의 타입·전체 lint·production 재빌드와 공개 실제 조작 후속 확인을 진행한다. 제품 변경이 없으므로 새 배포는 하지 않는다.
- 정상 원본 복구 후 타입·전체 lint·production build가 모두 통과했다. 실행 중 검사와 빌드를 겹치지 않았다. 새 제품 논리가 없으므로 단위검사203개를 이번에 다시 실행했다고 보고하지 않는다. 독립 해석 `C:\TEMP\cg-storage-stage-sources-20260924\MEASUREMENTS.md`도3회 로그를 대조해 최초 실패 유지와 trace 단독 원인 미확정을 확인했다.
- 공개 URL 최종 스모크 **14/14 통과(3.5분)**: Chromium390×844의9개 게임 설정→실제 문제 화면→종료 확인→원래 카드 초점, 모바일 Safari5개의 키보드/native 창·터치/준비센터·무점수 예제→첫 문제·v2/v3 기록 이전을 확인했다. 기존 timeout/trace/video 설정 유지·retry0·worker1이다. `C:\TEMP\cg-smoke-production-2200-20260924`와 형제 `-report`, `-console.log`. 시작·종료 스모크는9게임 전체 세션 완료나 실제 iPhone 검증이 아니며, 앞선 진단 실패를 취소하지 않는다.
- 기존 사용자 IAB에서는 진행 중 게임이 없는지 확인한 뒤 NBack 설정 열기→Esc 닫기를 직접 수행했다. 기존 기록5개 유지·원래 카드 초점 복귀·stage0/inert0·console error0을 확인했다. 사용자 탭에서 게임 완료·기록 추가/삭제·설정 변경 없음. 지정 alias 재조회도 `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP` 유지·HTTP200/redirect0이다. 새 외부 페이지/배포 변경은 없다.
- 이번 원본 수정은 이 문서1개뿐이며 신규 프로젝트 파일은 없다. 생성한 임시 진단/계측 보존본과 분석·검사 산출물은 위 C:\TEMP 경로에 남겼다. 소유한4187 서버·테스트·빌드와 임시 탭은 종료했다. 문서만 커밋/push하고 재배포하지 않는다. 다음 heartbeat는 문서 CI/Git/alias 확인 뒤 재현 조건이 같은 속도검사를 무작정 반복하지 않고,9게임의 입력·일시정지·결과 저장에서 검증 공백이나 새 실제 결함이 있는지 우선한다. 안정된 환경/실제 Safari 성능 측정은 여전히 남은 한계다. 자동화22 ACTIVE·자정 종료 유지.

## 22:42 사용자 계속 요청 — 길 만들기 반복 키 입력 보호

- 22:42:49 KST 시작. 원본 `77ea733` clean·origin 확인, 문서 CI36005234049 성공. 지정 alias는 `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP` 유지·HTTP200/redirect0 확인. WebKit 지연을 같은 조건으로 반복 진단하지 않고 실제 입력/결과 경로를 분담해 읽었다. 자정 종료 조건은 유지한다.
- PathGame의 키보드 격자 이동 함수2개에는 배치 변경 키의 `event.repeat` 방어가 없었다. 분리된 공개 Playwright 컨텍스트에서 keyup 없는 `keyboard.down('Enter')`2회로 **RED2/2** 재현:1280px는 처음 설치한 `/`가 없어지고390px는 `\\`로 바뀌었다. 첫 설치/조작1회 확인 뒤 두 번째 입력의 배치 유지 assertion이 실패했다. 증거 `C:\TEMP\cg-path-repeat-red-20260924` 및 형제 `-report`, `-console.log`. 단순 속도 초과나 가상 클릭이 아닌 실제 브라우저 반복 keydown이다.
- 최소 제품 수정은 `app/components/game-stage.tsx`의3곳이다. 넓은 화면은 Enter/Space 반복 활성화, 작은 화면은 추가로 Slash/Backslash 반복 배치를 차단했다. 전체 초기화 버튼도 기존 경로 확인 버튼과 같은 반복 활성화 방어를 적용했다. 최초 입력·키를 뗀 뒤 새 입력·방향키 반복 이동·마우스/터치·게임 규칙/타이머/채점식은 변경하지 않았다. 새 추상화/라이브러리 없이 기존 입력 방어 패턴을 재사용했다(Ponytail lite).
- 신규 `e2e/path-repeat-input.spec.ts`2개 시나리오(1280×900,390×844)를 Chromium과 기존 Desktop WebKit 프로젝트에 등록했다. Enter 반복/새 입력·compact 슬래시2종·방향키 반복 이동·Space 단발 및 held Space keyup·Tab으로 제출/초기화 접근·초기화 조작 수 중복 방지·콘솔 오류를 검증한다. 독립 읽기 검토에서 P1/P2 회귀는 발견하지 못했으며 직접 실행은 별도로 확인한다.
- 추가 읽기 후보는 결과 복습 버튼에서 Enter를 유지할 때 닫기 버튼으로 옮겨간 초점의 반복 입력, 게임 lazy 로딩 중 취소 동작 공백, Potion 마지막 피드백 중 pause/resume→단일 저장이다. 이들은 실제 재현 전이므로 확정 오류/완료로 보고하거나 이번 Path 수정에 섞지 않는다. 현재 C:\TEMP 타입·lint·단위·build 후 Path 새 입력/기존 반응형·완료 검사를 진행 중이며, 아직 제품 커밋/공개 배포 전이다. 이 변경과 같은 파일을 중복 수정/배포하지 않는다.
- 23:06 KST 확인: 타입·전체 lint·단위 **203/203**·production build 모두 통과. 로컬 production 서버에서 Path 새 반복 입력4개(Chromium2/Desktop WebKit2), 기존 완료·오답 수정2개, 반응형 초점 전환2개가 **8/8 통과(2.0분)**했다. Space 반복 down→down→up도 양 엔진에서 단일 조작으로 끝났고, 새 Enter 입력과 방향키 반복 이동은 유지됐다. 증거 `C:\TEMP\cg-path-repeat-local-20260924` 및 형제 `-report`, `-console.log`. G:와 C:\TEMP 코드/설정/신규 검사 SHA256 일치·diff check 통과, 임시 탭/소유4187 서버 종료 완료. 제품 선택 파일4개만 커밋/push 후 전체 CI 성공과 후보 검증을 확인한 뒤 지정 공개 alias를 갱신한다.
- 제품 `34e3dde` main push 완료. 기존 프로젝트 후보 `dpl_3FK5RuLGrz48cWFveVKBbMhPsAXF` / `https://competency-game-m5j2l5ym2-heotaewoongs-projects.vercel.app`를 한 번 생성했다. READY·commit metadata 일치·인증된 HTTP200 확인. 지정 heobrain alias는 이전 `dpl_PTsaSKnKqXKkWNue7ZdRqx7BgdCP` 유지. [CI36010543379](https://github.com/heotaewoong/competency-game/actions/runs/36010543379)의 타입·전체 lint·단위 단계 성공 후 전체 브라우저 검사 진행 중이다. 독립 최종 읽기 검토에서도 Space 추가 검증과 기대 조작 수 정합성을 확인했으며 출시 차단 문제는 발견되지 않았다.
- CI 대기 중 공개 이전 제품의 Potion 마지막 피드백 정지/재개 경계를 별도 Chromium 진단으로 확인해 **1/1 통과(21.2초)**했다. 실제 28회 클릭과 공개 제조색 관찰→마지막 피드백400ms→종료 확인 모달5초→계속 연습599ms까지 미완료→1ms 후 완료→저장 및 새로고침을 수행했다. 결과/저장1개, 응답28개, 복습 index0..27/고유ID28개, 실제 색 적중 집계 및 근거 채점 대상 수가 일치했고 추가5초 뒤 저장도 동일·runtime error0이었다. 저장 시도 중 근거 없는 초기 학습 시행은 복습에 보존하되 근거 채점에서 제외되는 현재 사양을 확인했다. 가상시계 Chromium 검증이지 실제 시간/모든 브라우저 무오류 보장이 아니며 Potion 앱 변경은 없다. 증거 `C:\TEMP\cg-potion-final-pause-20260924` 및 형제 `-report`, `-console.log`; 임시 spec은 검사 완료 후 SHA256을 확인해 `C:\TEMP\cg-potion-final-pause-sources-20260924\potion-final-pause-diagnostic.spec.ts`로 이동·보존했다. G: 신규 테스트나 CI 목록에는 추가하지 않았다.
...
- 제품 `d36b8950408273ad321729eba7dedfd912657fd1`을 main에 push했다. [CI36122592711](https://github.com/heotaewoong/competency-game/actions/runs/36122592711)의 타입·lint·단위 단계 성공 후 전체 브라우저 검사가 진행 중이다. 아직 전체 CI 성공으로 표현하지 않는다.
- 깨끗한 main 복제본 `C:\TEMP\competency-game-release-20260925`에서 기존 Vercel 프로젝트에 `--prod --skip-domain` 후보를 한 번 생성했다. `dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6` / `https://competency-game-gugsqaclu-heotaewoongs-projects.vercel.app`가 READY이며 commit metadata는 `d36b895`와 일치한다. 인증된 HEAD HTTP200을 확인했다. 자동으로 붙은 프로젝트 기본 alias와 별개로, 지정 `heobrain-competency-game.vercel.app`는 API 재조회에서 이전 `dpl_3FK5RuLGrz48cWFveVKBbMhPsAXF`를 유지했다. CI 성공 전 promote하지 않았다.
- 별도 IAB 로컬 탭 생성 첫 시도는 도구 응답 시간 초과로 실패했다. 재고 조회로 생성된 로컬 탭이 없음을 확인한 후 한 번 다시 생성해 정상 접근했다. 1280×720 화면에서 실제 footer 복습 클릭→빈 복습 창/닫기 초점/화면 확인→Escape→원래 footer 버튼 초점 복귀를 직접 확인했고 콘솔 warning/error 배열은 비었다. 첫 클릭 직후 AX에는 아직 창이 없었지만 다음 화면 관찰에서 표시됐으며 재클릭하지 않았다. 이 수동 Chromium 관찰을 WebKit 또는 실제 iPhone 결과로 대체하지 않는다. 임시 탭은 닫았고 소유한4187 로컬 서버도 종료했다. 사용자 공개 탭/기록은 조작하지 않았다.
- CI36122592711은 **19:26:44 KST 성공(15분31초)**으로 완료됐다. SHA `d36b895`의 타입·전체 lint·단위 **203/203**, 전체 브라우저 **171/171(14.0분)** 통과를 완료 로그에서 확인했다. unit23개 파일 합산 fail0, E2E retry/flaky/failed 항목은 없었다. Actions Node 런타임/Ubuntu 이미지 예정 변경 안내는 별도 유지보수 항목이며 검사 실패가 아니다.
- 지정 alias가 여전히 직전 제품임을 재확인한 뒤 같은 후보를 한 번 promote했다. `heobrain-competency-game.vercel.app`의 API readback은 **dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6**, 공개 HTTP200·redirect0으로 일치한다. 추가 빌드/중복 후보는 만들지 않았다. 최종 공개 복습·초점 관련26개 검사를 별도 새 컨텍스트에서 진행 중이며, 이 시점에는 완료로 표현하지 않는다.
- 지정 공개 주소의 최종 관련 E2E가 **26/26 통과(4.0분)**했다. Chromium13·Desktop WebKit11·Mobile Safari2, 로컬과 같은 검사/기본 timeout·retry0·worker1이며 실제 Enter 반복/새 입력·Space·포인터/터치·Escape·opener 초점·중첩 배경 차단·게임 handoff와 오류 배열 검사를 통과했다. 증거는 `C:\TEMP\cg-review-focus-production-20260925-1930` 및 형제 `-report`, `-console.log`에 보존했다. Node 출력의 NO_COLOR/FORCE_COLOR 안내는 제품 콘솔 오류가 아니다. 일부 공개 검사 소요가 로컬보다 길었으며 이번 통과를 초기 WebKit 지연 원인 규명이나 전반적 성능 해결로 표현하지 않는다.
- 이번 공개 제품은 **d36b895 / dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6**이다. 원본 수정 파일은 `app/components/game-stage.tsx`, `app/components/review-dialog.tsx`, `app/page.tsx`, `playwright.config.ts`, 본 문서5개이며 생성한 원본 파일은 `e2e/review-held-enter.spec.ts`, `e2e/review-pointer-focus.spec.ts`2개다. 외부 변경은 GitHub main과 기존 Vercel 프로젝트/지정 공개 주소다. 사용자 브라우저 기록·게임 규칙·타이머·점수식·전역 설정·자격증명은 수정하지 않았다. Ponytail lite에 따라 기존 공통 키 처리와 opener 패턴을 재사용했으며 새 의존성/추상화는 추가하지 않았다.
- CI 모니터·공개 검사·배포 프로세스는 정상 종료했고 검증용 로컬 서버와 임시 탭도 정리했다. 자동화22는 PAUSED를 유지한다. 후속은 이 결과 문서만 커밋/push하며 제품 재배포하지 않는다. 실제 iPhone 하드웨어, 초기 WebKit 지연의 원인 및 비공개 JOBDA 현행 사양은 여전히 미검증/미확정이다. 과거 실패 증거는 그대로 보존했고 이번 성공으로 삭제하거나 성공으로 바꾸지 않았다.

## 2026-09-25 23:55 사용자 요청 — 지금부터 6시간 개선 재개

- 사용자가 '계속 업데이트 진행 지금부터 6시간 동안'을 요청했다. 첫 시각 확인 **2026-09-25 23:55:18 KST**를 기준으로 종료는 **2026-09-26 05:55:18 KST**다. 이전07:00/자정 종료 조건과 직전 PAUSED 상태보다 이 요청을 우선한다.
- 기존 자동화22를 동일 작업·기존30분 간격으로 ACTIVE 재개했다. 새 자동화는 만들지 않았다. 매 실행 시작과 새 변경/검사/커밋/push/배포/승격 전에 시각을 확인하고, 종료 시각 이후 새 제품 작업 없이 결과 정리와 PAUSED 처리만 하도록 저장했다. 도구 응답과 저장된 automation.toml에서 이름·ACTIVE·30분 간격·대상 작업·종료 시각을 재확인했다. 로컬 파일 작업에는 PC와 앱이 실행 중이어야 한다.
- 시작 상태: G: HEAD와 GitHub main **5bf4a55b1f873b92b0021d675fb2b43b15f275cf**, 원본 clean·기존 remote 일치. 최신 문서 [CI36124828291](https://github.com/heotaewoong/competency-game/actions/runs/36124828291)는 success이며 완료 로그의 타입/lint·단위203/203·브라우저171/171(12.9분), retry/flaky 없음까지 읽기 전용 확인했다. 앞선 제품 CI 성공과 이번 새로운 테스트 실행은 구분한다.
- Vercel CLI 지침에 따라 기존 프로젝트 연결을 확인하고 지정 alias를 재조회했다. 공개 주소는 **dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6** 유지·HTTP200/redirect0이다. 제품 d36b895의 복습 수정은 이미 반영됐으므로 오래된 미커밋 우선순위를 자동화에서 제거했다. 진행 중 프로젝트 제품 검사/서버/배포는 관측되지 않았고 사용자 프로그램은 조작하지 않았다.
- 후속 우선순위는 게임 로딩 취소·입력 경계·일시정지/중복 저장·반응형의 확인된 검증 공백이다. 재현 전 후보는 확정 오류로 보고하지 않으며, 초기 WebKit 지연은 새 근거 없이 같은 조건의 검사를 반복하거나 timeout을 늘리지 않는다. 작은 기존 패턴 재사용→검사→main push/CI→기존 후보 검증/공개 반영→지정 URL 실제 조작 순서를 유지한다. 유의미한 수정·배포·실패·사용자 조치와 최종 결과만 알린다.
- 이번 재개 설정에서 수정한 프로젝트 파일은 본 문서1개이며 생성한 프로젝트 파일은 없다. 앱 코드·게임 규칙·타이머·사용자 데이터 변경, 새 단위/E2E/build 실행, Git 커밋/push 또는 Vercel 배포는 하지 않았다. 외부 변경은 Codex 자동화22의 재개/종료 조건뿐이다. 이 로컬 재개 기록은 다음 검증된 변경의 진행 기록과 함께 관리한다.

## 2026-09-26 00:31 재개 — 느린 게임 로딩 취소

- 시작 시각과 05:55:18 KST 종료 조건을 재확인했다. HEAD `5bf4a55`·remote·공개 alias `dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6`·HTTP200/redirect0 유지. 기존 제품 복습 수정은 재작업하지 않았다. G: tracked 파일과 검증본 `C:\TEMP\competency-game-resume-20260925-1849`의 SHA256 차이는 위 재개 문서뿐이었으며 새 앱/검사4파일은 수정 후 다시 복사했다.
- 공개 홈의 hydration 완료 후 새 게임 JS 다운로드만 보류하고 실제 도형 회전 카드→Escape를 조작했다. 로딩 화면이 닫히지 않아 신규 회귀검사 Chromium escape **1실패(15.1초)**로 기존 문제를 재현했다. 증거 `C:\TEMP\cg-stage-loading-red-20260926` 및 형제 report/console.log를 보존했다. 명시 캡처 `game-loading.png`는 취소 수단 없는 로딩 상태이며, 자동 실패 캡처는 finally에서 다운로드를 해제한 뒤 게임 화면이므로 실패 시점 화면과 구분한다. 최초 WebKit 지연 원인과는 별개다.
- `app/page.tsx`의 로딩 fallback을 취소 버튼/Escape가 있는 native dialog로 바꾸고 기존 `lockDocumentScroll`을 재사용했다. React lazy/Suspense로 fallback에 기존 close 동작을 전달하며 게임의 지연 다운로드와 사전 로딩은 유지한다. [Next.js 공식 lazy-loading 안내](https://nextjs.org/docs/app/guides/lazy-loading)의 지원 API 범위다. 버튼 반복 Enter/Space 기본 동작을 차단해 눌린 채 진입한 키가 즉시 취소하지 않도록 했다. `app/globals.css`의 로딩 안내/44px 버튼/좁은 화면 크기를 보완했다. 게임 규칙·타이머·점수·저장식·의존성 변경 없음.
- 새 `e2e/stage-loading.spec.ts`는 Escape/포인터·터치/Enter 반복/정상 다운로드4경로에서 초점·스크롤 복구·늦게 끝난 import의 재진입 방지·실제 게임 재진입·기록 불변·콘솔/runtime 오류를 점검한다. WebKit와 모바일 WebKit도 config에 등록했다. 기본 timeout/retry는 변경하지 않았다. 현재 타입·lint 통과, 새 production build 진행 중이며 아직 수정 후 브라우저 검사/공개 반영을 완료로 보고하지 않는다.
- 타입·전체 lint·production build·단위 **203/203** 통과. 새 BUILD_ID `rIAqasw8ETbzGgYjf75wf`, 원본과 검증본의 앱/자산/검사/설정111개 SHA256 불일치0, `git diff --check` 통과. 첫 Chromium Escape 재현 검사는 수정 후 **1/1(8.1초)** 통과했고 취소 화면 스크린샷도 직접 확인했다. `C:\TEMP\cg-stage-loading-green-first-20260926`.
- 첫 관련29개 실행은 **14통과/1실패/14미실행(1.0분)**이다. Desktop WebKit 첫 게임 진입 전 `#records[aria-busy]`가5초 안에 false가 되지 않았다. 게임 로딩 fallback은 아직 열리지 않은 시점이다. `C:\TEMP\cg-stage-loading-local-20260926`에 trace/video/screenshot을 보존했다. trace상 JS 요청14개가 모두200이고 개별 다운로드21~66ms, 접근성 bootstrap marker가 있으며 page/console error는 관측되지 않았다. 따라서 다운로드 실패는 확인되지 않았지만 effect/브라우저 스케줄링 지연의 원인은 미확정이다.
- 제품을 더 바꾸지 않고 C:\TEMP 전용 진단에서 requestAnimationFrame 요청/콜백·문서 상태·저장 키·JS 다운로드를 관찰했다. 별도 WebKit 로컬/공개 **2/2(14.6초)**에서 기록 복원/RAF가 정상 완료됐다(관찰시점1.32초/1.40초, 오류0). `C:\TEMP\cg-hydration-diag-webkit-20260926` 및 console.log. 최초 진단 CLI는 config projects와 `--browser` 병용 오류로 실행 전 종료됐으며 별도 진단 config로 바로잡았다. 진단 파일은 G:나 제품 커밋에 포함하지 않는다. 새 관측을 바탕으로 같은 기본 timeout/retry0에서 관련29개를 한 차례 다시 확인하며, 최초 실패를 삭제하거나 지연 원인이 해결됐다고 주장하지 않는다.
- 최종 로컬 관련 검사 **29/29(3.5분)** 통과: Chromium14·Desktop WebKit11·Mobile Safari4. 지연 다운로드의 Escape/실제 포인터·터치/held Enter/정상 완료12개, 기존 홈 opener·가이드/복습 handoff14개, 기록 초기 로딩/저장소 차단3개다. `C:\TEMP\cg-stage-loading-local-final-20260926` 및 형제 report/console.log. 데스크톱1280×720 및 모바일390×844 로딩 캡처에서 안내·취소 버튼이 화면 안에 표시됨을 직접 확인했다. 이 결과를 실제 iPhone 하드웨어 검증이나 모든 지연 해소로 확대하지 않는다.
- 선택한 제품2파일(`app/page.tsx`, `app/globals.css`)·검사 설정·신규 회귀검사·본 문서만 커밋/push한다. 전체 CI 성공→기존 Vercel 프로젝트 검증 후보 승격→지정 공개 URL 실제 검증 전에는 공개 완료로 보고하지 않는다. 진단용 C:\TEMP 파일과 과거 실패 증거는 제품에 포함하지 않는다.
- 제품 **9b35ffade75f665ce1226233752a44c420788e7e**를 main에 push했다. [CI36157842024](https://github.com/heotaewoong/competency-game/actions/runs/36157842024)의 타입·lint·단위 단계는 성공했고 전체 브라우저 검사가 진행 중이다. 같은 SHA/clean을 확인한 기존 C:\TEMP 릴리스 복제본에서 후보 **dpl_77skTQy3oeASdHnzTK781JBKyfsw** / `https://competency-game-6x7qvdops-heotaewoongs-projects.vercel.app`를 한 번 만들었다. READY·commit metadata 일치·인증 HEAD200, build27초다. 지정 HeoBrain alias는 API readback에서 이전 dpl_9dUnNnciMKTmXM3dnnkbSWsDoDi6 유지. CI 성공 전 승격하지 않았다.
- 대기 중 C:\TEMP 전용 작은 화면 진단 **2/2(7.7초)** 통과: 320×568·844×360에서 로딩 dialog/버튼 전체 경계가 viewport 안에 있고 버튼 높이44px 이상이며 모달 뒤 버튼으로 초점을 보내도 차단되고 취소 후 원래 버튼으로 복귀했다. 두 캡처도 직접 확인했다. `C:\TEMP\cg-loading-layout-20260926` 및 report/console.log. 이 진단은 기존 Playwright를 재사용하며 제품 파일 추가/수정은 하지 않았다.
- CI36157842024가 **01:16:08 KST 성공**으로 완료됐다. 완료 로그에서 단위23파일 **203/203**, 전체 브라우저 **183/183(15.1분)**, failed/flaky/retry 없음까지 확인했다. 로그 `C:\TEMP\cg-stage-loading-ci-36157842024-completed.log`. Actions Node20 런타임/ubuntu-latest 이미지 변경 안내는 제품 검사 실패가 아니며 별도 유지보수 항목이다.
- 지정 alias가 이전 제품임을 재확인하고 검증한 같은 후보를 한 번 promote했다. API readback **dpl_77skTQy3oeASdHnzTK781JBKyfsw**, 지정 공개 HTTP200/redirect0. 새 빌드/중복 후보를 만들지 않았다. 공개 관련29개 검사를 별도 컨텍스트에서 진행 중이다. 배포별 최근1시간 error 로그 조회는 `No logs found`였으며 정적 클라이언트의 무오류나 모니터링 완비를 의미하지 않는다. 사용자 탭/기록은 조작하지 않았다.
- 검증 서버 PID45904의 명령 경로가 위 C:\TEMP 검증본 Next start/4187임을 재확인하고 해당 서버만 종료했다. 이 의도적인 종료로 서버 셸 exit1이 반환된 것은 build/test 실패가 아니다. 다른 사용자 프로그램/기존 실패 증거는 유지했다.
- 첫 공개 관련 실행은 **17통과/1실패/11미실행(1.7분)**이다. Desktop WebKit의 '기록 다시 연습' 진입 전 `#records aria-busy`5초 실패가 재발했다. `C:\TEMP\cg-stage-loading-production-20260926`에 원본 증거를 보존했다. 초기JS14개는 모두 HTTP200/개별66~68ms, bootstrap marker 있음/page·console error 없음이다. 취소 기능 실행 전 실패를 취소 기능 성공이나 전체 공개 통과로 바꾸지 않는다.
- 추가 C:\TEMP 전용 WebKit 계측은 기록0개/1개 각2회 **4/4(28.4초)**에서 복원과 RAF 콜백이 완료됐다. 관찰시점1.31~1.88초, RAF 대기5~314ms, 각 저장소 읽기9회, 오류0. `C:\TEMP\cg-hydration-production-diag-20260926` 및 console.log. 이 계측은 Storage getItem/RAF를 관찰한 별도 조건이므로 원래 회귀검사 통과를 대체하지 않는다. 원인 미확정을 유지하고 새 관측 후 원래29개·동일 timeout/retry0를 한 차례 더 확인한다. 같은 실패를 무제한 재실행하거나 계측 파일을 제품에 반영하지 않는다.
- 최종 공개 관련 검사는 **29/29(4.3분)** 통과했다. Chromium14·Desktop WebKit11·Mobile Safari4, 원래 검사와 동일한5초 assertion/기본 action·navigation timeout/retry0·worker1이며 실제 취소/키 반복/재진입/opener 복구/기록 불변/콘솔·runtime 오류 배열 검사를 통과했다. `C:\TEMP\cg-stage-loading-production-final-20260926` 및 report/console.log. 공개 모바일 held-Enter 로딩 캡처도 직접 확인했다. 이 성공으로 앞선 로컬·공개 초기 복원 지연 실패를 삭제하거나 원인이 해결됐다고 주장하지 않는다. 실제 iPhone 하드웨어·비공개 JOBDA 현행 사양은 검증 범위 밖이다.
- 최종 제품은 **9b35ffa / dpl_77skTQy3oeASdHnzTK781JBKyfsw**, 공개 주소는 `https://heobrain-competency-game.vercel.app/`다. 원본 수정 파일4개는 `G:\내 드라이브\0.project_money\competency-game\app\page.tsx`, `G:\내 드라이브\0.project_money\competency-game\app\globals.css`, `G:\내 드라이브\0.project_money\competency-game\playwright.config.ts`, 본 문서다. 생성한 원본 파일1개는 `G:\내 드라이브\0.project_money\competency-game\e2e\stage-loading.spec.ts`다. 외부 변경은 GitHub main·기존 Vercel 프로젝트/지정 공개 주소·자동화22의 다음 실행 기준이다.
- C:\TEMP 전용 생성 진단 파일은 `C:\TEMP\competency-game-resume-20260925-1849\hydration-diagnostic.config.ts`, `C:\TEMP\competency-game-resume-20260925-1849\e2e\hydration-diagnostic.spec.ts`, `C:\TEMP\competency-game-resume-20260925-1849\e2e\loading-layout-diagnostic.spec.ts`이며 제품에 포함하지 않았다. 실행 결과·이미지·trace는 위 명시한 각 증거 폴더에 보존했다. Ponytail에 따라 기존 React lazy/Suspense·native dialog·scroll lock을 재사용했으며 새 라이브러리/추상화·게임 규칙/타이머/점수 변경은 없다.
- 자동화22는 기존30분 간격/05:55:18 종료/ACTIVE를 유지하고, 완료된 로딩 취소를 재작업하지 않도록 최신 제품·CI/공개 결과와 진단 파일 구분을 저장했다. 다음 우선 과제는 초기 WebKit 기록 복원 지연의 새 근거 확보 및 입력/일시정지/저장 검증 공백이다. 배포/공개 검사/CI 모니터는 완료됐고 검증 서버도 종료됐다. 이번 최종 기록만 후속 커밋/push하며 문서 전용 변경으로 제품을 재배포하지 않는다.
