...
- 검사 이야기: 게임 완료 → v4 기록 저장 → 새로고침 → 해당 게임 문항 복습 복원. 기존 9종 완료 검사의 공통 저장 확인 뒤 새로고침·복습 열기를 추가했다. 제품 저장 로직을 바꾼 것은 아니며 검사 실행 전이다.
- 읽기 전용 병렬 감사에서 2개 후보 확인: 길 만들기의 900px 경계 전환 시 숨겨진 버튼으로부터 키보드 포커스 복구 누락(P2 후보), 회전 과정 재생의 수동 정지 시 남은 단계 시간 미보존(P3 후보). 아직 실제 재현 전이므로 확정 오류나 수정 완료로 보고하지 않는다.
- 현재 소유 파일: 루트 `e2e/completion-flows.spec.ts`, UX 감사 `e2e/path-responsive-focus.spec.ts` 신규, 타이머 감사 `e2e/timer-boundary-regressions.spec.ts`. 원본·검증본에 테스트만 추가했으며 앱 코드는 아직 변경하지 않았다. 공개 URL의 독립 브라우저 테스트로 재현 후 수정 여부를 결정한다.
...
- 검증본의 포트 4173은 다른 프로세스가 사용 중이므로 건드리지 않았다. 이 작업 전용 production server는 `127.0.0.1:4187`(루트 세션 67019). 신규 회귀·9종 완료/재방문 복습·성능·Mobile Safari 총22개 실행 중(루트 세션37617), 완료 전 중복 실행/배포하지 않는다.
