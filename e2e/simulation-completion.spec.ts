...
test('가위바위보 실전형은 3라운드 30문항과 제한시간을 끝까지 유지한다', async ({ page }) => {
  ...
  await expectSimulationResult(page, game, '오류', 30, pageErrors);
});
...
test('도형 순서 기억하기 실전형은 두 라운드 47개 채점 문항을 모두 진행한다', async ({ page }) => {
  ...
  for (let index = 0; index < 52; index += 1) {
    ...
    await expect(nbackStage.locator('.nback-actions button')).toHaveCount(actionCount);
  ...
  await expectSimulationResult(page, game, '오류', 47, pageErrors);
  await expect(page.locator('.nback-result-detail')).toContainText('시간 초과47');
});
